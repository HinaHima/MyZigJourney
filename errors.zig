...
test "error union" {
    const maybe_error: AllocationError!u16 = 65535;
    const no_error = maybe_error catch 0;

    try expect(@TypeOf(no_error) == u16);
    try expect(no_error == 65535);
}

// --------------------------------------------------------
// Functions often return error unions. Here's one using a catch, where the |err| syntax receives the value of the error.
// This is called payload capturing, and is used similarly in many places. We'll talk about it in more detail later in the chapter.
...
// --------------------------------------------------------
// try x is a shortcut for x catch |err| return err, and is commonly used where handling an error isn't appropriate.
